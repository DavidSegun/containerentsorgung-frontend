  ...
  return sdk.client
    .fetch<{
      ...
      query: {
        ...
        offset,
        fields:
          "*variants,*variants.prices,+variants.inventory_quantity,*seller,*variants,*seller.products," +
...
export const listProductsWithSort = async ({
  ...
  if (tag_id) {
    productQueryParams.tag_id = [tag_id]
  }
  ...
  const pricedProducts = filteredProducts.filter((prod) =>
    prod.variants?.some((variant) => (variant as any).calculated_price !== null || ((variant as any).prices && (variant as any).prices.length > 0))
  )
