    ...
    await sdk.store.cart
      .updateLineItem(
        ...
        { 
          ...
          metadata: metadata || (currentItem.metadata as Record<string, unknown>) || undefined,
        },
        {},
        headers
      )
 ...
 * @param locale - optional - The locale for the redirect URL.
 * @returns The cart object if the order was successful, or null if not.
 */
export async function placeOrder(cartId?: string, locale?: string) {
  const id = cartId || (await getCartId())
  ...
  if (cartRes?.order_set) {
    ...
    const redirectPath = locale 
      ? `/${locale}/order/${cartRes?.order_set.orders[0].id}/confirmed`
      : `/order/${cartRes?.order_set.orders[0].id}/confirmed`
