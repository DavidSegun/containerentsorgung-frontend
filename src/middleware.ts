...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    // Define protected sections that require auth (keep /:locale/user accessible for login UI)
    const protectedSections = new Set(["checkout"]) // add more like "orders" if needed

    const isProtected = protectedSections.has(section)
    const isLoginPage = section === "user"

    if (!jwt && isProtected && locale) {
      const loginUrl = new URL(`/${locale}/user`, request.nextUrl.origin)
      // Optional: preserve original destination
      loginUrl.searchParams.set("redirect", request.nextUrl.pathname + request.nextUrl.search)
      return NextResponse.redirect(loginUrl, 307)
    }
  } catch (e) {
    // fail open on auth guard errors
  }
